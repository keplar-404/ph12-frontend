...
import { addDonation, updateDonaton } from "../../services/api/axios/axios";
import { useFormik } from "formik";
import InputField from "./InputField";
import Textarea from "./Textarea";
import { addDoationSchema } from "../../libs/schema";
import { handleFileValidation } from "../../utils/validation";

export default function DonationForm({ initialValues }) {
  ...
  const onSubmit = async (value, action) => {
    ...
    if (initialValues) {
      ...
      await updateDonaton(initialValues, Photo);
      await setIsSubmittiong(false);
      return;
    }

    await addDonation(value, userData?.id, Photo);
    await setIsSubmittiong(false);
  };
  ...
  const { values, errors, handleBlur, handleChange, handleSubmit, touched } =
    useFormik({
      initialValues: {
        ...
        maximumDonationAmount: initialValues?.maximumDonationAmount || "",
        highestAmountUserCanDonate:
          initialValues?.highestAmountUserCanDonate || "",
        lastDateOfDonation: initialValues?.lastDateOfDonation || "",
        shortDescription: initialValues?.shortDescription || "",
        longDescription: initialValues?.longDescription || "",
      },
      validationSchema: addDoationSchema,
  ...
  const inputField = [
    ...
    },
    {
      ...
      type: "number",
      errors: errors.maximumDonationAmount,
      placeholder: "Maximum Donation Amount",
      values: values.maximumDonationAmount,
      touched: touched.maximumDonationAmount,
    },
    {
      id: "highestAmountUserCanDonate",
      type: "number",
      errors: errors.highestAmountUserCanDonate,
      placeholder: "Highest Amount User Can Donate",
      values: values.highestAmountUserCanDonate,
      touched: touched.highestAmountUserCanDonate,
    },
    {
      id: "lastDateOfDonation",
      type: "date",
      errors: errors.lastDateOfDonation,
      placeholder: "Last Date Of Donation",
      values: values.lastDateOfDonation,
      touched: touched.lastDateOfDonation,
    },
    {
      id: "shortDescription",
      type: "text",
